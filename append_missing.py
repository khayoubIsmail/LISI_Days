...
for line in lines:
    ...
    if len(parts) >= 2:
        ...
        # Match ignoring order of names
        name_words = set(name_only.lower().split())
        match = False
        for ex in existing_names:
            ex_words = set(ex.lower().split())
            if len(name_words.union(ex_words)) < len(name_words) + len(ex_words) - 1:
                # If they share at least 2 words or something, or one contains the other
                pass
            # simpler:
            if all(w in ex.lower() for w in name_words) or all(w in name_only.lower() for w in ex_words):
                match = True
                break
            # if one word is matched entirely e.g. "Lafham"
            if len(name_words.intersection(ex_words)) >= 1 and len(name_words) <=2 and len(ex_words) <= 2:
                # e.g. Mohamed LAFHAM vs LAFHAM Mohamed -> 2 intersection
                if len(name_words.intersection(ex_words)) == len(name_words):
                    match = True
                    break
    ...
    new_cards += f'''
          ...
          <div class="speaker-card">
            <div class="speaker-image-circle">
              <img src="assets/images/comite/homme.png" alt="{m['name']}">
            </div>
...
# inject right before </div> <!-- end of speakers-grid -->
grid_end_idx = content.find('</div>\n\n      </div>\n    </section>\n\n    <!-- Section Programme -->')
if grid_end_idx == -1:
    # try another way
    grid_end_idx = content.find('</section>\n\n    <!-- Section Programme -->')
    # reverse find </div>
    idx = content.rfind('</div>', 0, grid_end_idx)
    grid_end_idx = content.rfind('</div>', 0, idx)

content = content[:grid_end_idx] + new_cards + content[grid_end_idx:]

with open(html_file, 'w', encoding='utf-8') as f:
    f.write(content)

print(f"Appended {len(missing_docs)} missing docs.")
